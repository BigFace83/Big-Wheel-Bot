...
void loop() {
  ...
  int V = ((255-abs(XCommand*0.75))*(float(YCommand)/255)) + YCommand; //Convert joystick positions to wheel speeds
  int W = ((255-abs(YCommand))*(float(XCommand*0.75)/255)) + XCommand*0.75;
  
  int RightWheelSP = (V+W)/2;
  int LeftWheelSP = (V-W)/2;

  if (RightWheel < RightWheelSP){
    RightWheel += 20;
    RightWheel = constrain(RightWheel, -255, 255);
  }
  else if (RightWheel > RightWheelSP){
    RightWheel -= 20;
    RightWheel = constrain(RightWheel, -255, 255);
  }
  analogWrite(enA,abs(RightWheel));

  if (LeftWheel < LeftWheelSP){
    LeftWheel += 20;
    LeftWheel = constrain(LeftWheel, -255, 255);
  }
  else if (LeftWheel > LeftWheelSP){
    LeftWheel -= 20;
    LeftWheel = constrain(LeftWheel, -255, 255);
  }
  analogWrite(enB,abs(LeftWheel)); 
  ...
  if(currentMillis - previousMillis > 100) //use this to determine frequency of sending data
  {
      previousMillis = currentMillis;
  
      sprintf(data_packet, "%d,%d,%d,%d,%d",Leftsonar, Rightsonar, Yawangle, LeftWheel, RightWheel);
